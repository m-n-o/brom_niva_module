...
module fabm_niva_brom_bubble
   ...
   !variables allocated here  
    type(type_state_variable_id)         :: id_CO2g, id_DIC 
   !state variables dependencies
   !global dependencies
    type (type_dependency_id)            :: id_temp, id_salt, id_pres, id_depth
   !diagnostic variables
    type(type_diagnostic_variable_id)    :: id_P_B, id_flux1bubble, id_CO2g_dissolution, id_r_bub
   ! diagnostic dependencies
    type(type_dependency_id):: id_pCO2
   !Model parameters
    real(rk):: W_float !floating
    real(rk):: K_CO2g_diss, R, P_A, pi, g, TK, N_bub, sigma, Henry, Df 
  ...
  subroutine initialize(self,configunit)
   ...
   !Specific rates of biogeochemical processes
    call self%get_parameter(self%K_CO2g_diss,&
      'K_CO2g_diss', '[1/d]','K_CO2g_diss', default=100.0_rk)   
   ...
   !Register state variables
    call self%register_state_variable(self%id_CO2g,&
      'CO2g', 'mmol/m**3','CO2g',minimum=0.0_rk,vertical_movement=-self%W_float/86400._rk)
   ...
   !Register diagnostic variables 
    ...
    call self%register_diagnostic_variable(self%id_CO2g_dissolution,&
       'CO2g_dissolution','uM/d','CO2g_dissolution', output=output_time_step_integrated)
  ...
  subroutine do(self,_ARGUMENTS_DO_)
   ...
   !state variables
    real(rk):: CO2g, DIC
   !diagnostic variables
    real(rk):: r_bub, P_B, flux1bubble,CO2g_dissolution
   !increments and misc
     real(rk):: d_CO2g, Vol_tot, sigma_tau, sigma, Sc, k_B
     real(rk):: temp, salt, pressure, depth, pCO2, Henry_corr
   _LOOP_BEGIN_
   !Retrieve current variable values
   !state variables
    _GET_(self%id_CO2g,CO2g)
    ...
    Vol_tot=CO2g*0.001_rk*self%R*(self%TK+temp)/(self%P_A+depth*self%P_A/10.3_rk)
...
!kh_corrected = kh_theta * exp(coef_temp*(1/abs_temp - 1/298.15))* 1.e6 * pressure * koef_salt !Micromol/l
   ...
   ! gas-water transfer rate kB (m/s):
    ...
    ! pressure inside the bubble and one bubble dissolution:
    if(r_bub.gt.0.00000000001) then
      P_B = self%P_A + (1000._rk+sigma_tau)*self%g*depth + 2*self%sigma/r_bub 
      flux1bubble = 4._rk*self%pi*(r_bub)**2._rk *k_B &
        *(P_B*Henry_corr - 101325._rk/1000000.0_rk*pCO2*Henry_corr) !convert pCO2 from uatm into pascal
    else
    ...
    CO2g_dissolution = self%N_bub*flux1bubble
    d_CO2g = CO2g_dissolution

    _SET_ODE_(self%id_CO2g,-d_CO2g)
    _SET_ODE_(self%id_DIC, d_CO2g)

    _SET_DIAGNOSTIC_(self%id_r_bub,r_bub)
    _SET_DIAGNOSTIC_(self%id_P_B,P_B)
    _SET_DIAGNOSTIC_(self%id_flux1bubble,flux1bubble)
    _SET_DIAGNOSTIC_(self%id_CO2g_dissolution,CO2g_dissolution)
