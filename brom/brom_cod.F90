!-----------------------------------------------------------------------
! brom_cod is free software: you can redistribute it and/or modify it under
...
module fabm_niva_brom_cod
    ! brom_cod parameterizes  the Chemical Oxygen Demand, COD
    ! (https://en.wikipedia.org/wiki/Chemical_oxygen_demand) 
    ! OM mineralization, nitrification, and oxidation of reduced specied of S, Mn, Fe, 
    ! present in suboxic conditions. brom_cod consists of 1 state variables 
    ! ( in oxygen-units):! - CHON - is a theoretical organic compound CnHaObNc 
    ! that can be can be fully oxidized to inorganic nutrients 
    ...
    type,extends(type_base_model),public :: type_niva_brom_cod
    ...
    real(rk) :: r_CHON_nut_oxy,r_CHON_nut_nut
    real(rk) :: r_CHON_dom_oxy, r_CHON_dom_nut
    real(rk) :: Tda, beta_da, Wchon, Bu
    contains
    ...
    subroutine initialize(self,configunit)
        ! !INPUT PARAMETERS:
        class (type_niva_brom_cod), intent(inout), target :: self
        integer,                      intent(in)            :: configunit
        ! !LOCAL VARIABLES:
        real(rk),parameter :: d_per_s = 1.0_rk/86400.0_rk
        ! Store parameter values in our own derived type
        ! NB: all rates must be provided in values per day,
        ! and are converted here to values per second. CHON
        call self%get_parameter(&
             self%r_CHON_nut_oxy, 'r_CHON_nut_oxy', '1/d', &
             'Specific rate of CHON mineralization',&
             default=0.10_rk,scale_factor=d_per_s)
        call self%get_parameter(self%beta_da,'beta_da','nd',&
             'Coefficient for dependence of mineralization on t ',&
             default=20._rk)
        call self%get_parameter(self%Tda,'Tda','nd',&
            'Coefficient for dependence of mineralization on t ',&
            default=13._rk)
        call self%get_parameter(self%Wchon,'Wchon','m/s', &
            'vertical velocity of CHON (<0 for sinking)', &        default=-1.0_rk,scale_factor=d_per_s)
        call self%get_parameter(self%Bu,'Bu','nd', &
            'Burial coeficient for lower boundary',&
            default=0.25_rk)
        ! Register state variables
        call self%register_state_variable(&
             self%id_CHON,'CHON','mmol/m**3','CHON  oxidizable compound ',&
             0.0_rk, minimum=0.0_rk, vertical_movement=self%Wchon)
        ! Register link to external variables
        call self%register_state_dependency(&
             self%id_oxy,'Oxy','mmol/m**3','OXY')
    ...
    subroutine do(self,_ARGUMENTS_DO_)
        class (type_niva_brom_cod),intent(in) :: self
        _DECLARE_ARGUMENTS_DO_
        ! LOCAL VARIABLES:
        real(rk) :: CHON,oxy,nut,dom,t,iopt
        real(rk) :: doxy,dCHON
        ! Rates of biogeochemical processes
        real(rk) :: CHON_decay_ox            
        ! oxic mineralization of CHON and ammonification (1/d)
        ! real(rk) :: CHON_decay_denitr        
        ! suboxic mineralization of CHON (denitrification) (1/d)
        _LOOP_BEGIN_
    ...
    subroutine do_bottom(self,_ARGUMENTS_DO_BOTTOM_)
        ...
        _GET_(self%id_chon,chon)
        ! BURYING into the sediments, mmol/m2/s 
        !(sinking rates "Wxxx" are in m/s and positive upward)
