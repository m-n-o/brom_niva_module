!-----------------------------------------------------------------------
! brom_eq_constants is free software: you can redistribute it and/or modify it under
...
module fabm_niva_brom_eq_constants
  use fabm_types
  ...
  !Gas constant
  !Dickson et al. , (2007)
  real(rk),parameter:: gasconst_bar_cm3_o_mol_k=83.14472_rk
  real(rk),parameter:: t_k_zerodegc = 273.15_rk

  type,extends(type_base_model),public :: type_niva_brom_eq_constants
    type(type_diagnostic_variable_id):: id_Kc0,id_Kc1,id_Kc2,id_Kw,id_Kb
    type(type_diagnostic_variable_id):: id_Kp1,id_Kp2,id_Kp3,id_KSi
    type(type_diagnostic_variable_id):: id_Knh4,id_Kh2s,id_kso4,id_kflu
    type(type_diagnostic_variable_id):: id_tot_free
...
contains
  ...
  subroutine initialize(self,configunit)
    class (type_niva_brom_eq_constants), intent(inout), target :: self
    ...
    call self%register_diagnostic_variable(&
         self%id_Kc2,'Kc2','mol/kg-sol','[H+][CO3--]/[HCO3-]',output=output_none)
    ! Water
    call self%register_diagnostic_variable(&
         self%id_Kw, 'Kw','mol/kg-sol','[H+][OH-]/H2O',output=output_none)
    ! Boric acid
    call self%register_diagnostic_variable(&
         self%id_Kb, 'Kb','mol/kg-sol','[H+][B(OH)4-]/[B(OH)3]',output=output_none)
    ! Phosphoric acid
    call self%register_diagnostic_variable(&
         self%id_Kp1,'Kp1','mol/kg-sol','[H+][H2PO4-]/[H3PO4]',output=output_none)
    call self%register_diagnostic_variable(&
         self%id_Kp2,'Kp2','mol/kg-sol','[H+][HPO4--]/[H2PO4-]',output=output_none)
    call self%register_diagnostic_variable(&
         self%id_Kp3,'Kp3','mol/kg-sol','[H+][PO4---]/[HPO4--]',output=output_none)
    ! Silicic acid
    call self%register_diagnostic_variable(&
         self%id_KSi,'KSi','mol/kg-sol','[H+][H3SiO4-]/[Si(OH)4]',output=output_none)
    ! Ammonia
    call self%register_diagnostic_variable(&
         self%id_Knh4,'Knh4','mol/kg-sol','[H+][NH3]/[NH4+]',output=output_none)
    ! Hydrogen sulfide
    call self%register_diagnostic_variable(&
         self%id_Kh2s,'Kh2s','mol/kg-sol','[H+][HS-]/[H2S]',output=output_none)
    ! Hydrogen sulfate
    call self%register_diagnostic_variable(&
         self%id_kso4,'kso4','mol/kg-sol','[H+][HSO4-]/[H2SO4]',output=output_none)
    ! Hydrogen fluoride
    call self%register_diagnostic_variable(&
         self%id_kflu,'kflu','mol/kg-sol','[H+][F-]/[HF]',output=output_none)
    ! Ratio H_Tot/H_free
    call self%register_diagnostic_variable(&
  ...
  subroutine do(self,_ARGUMENTS_DO_)
    class (type_niva_brom_eq_constants),intent(in) :: self
  ...
  real(rk) FUNCTION ABETA_HF_DIRI79(t_k, s, p_bar) result(ans)
    ...
    !zcvt_to_kgsw   : mass of pure water in 1kg of seawater as a fct. of
    !                 salinity
  ...
  real(rk) FUNCTION AK_HSO4_DICK90(t_k, s, p_bar) result(ans)
    !adopted from Munhoven(2013)
    !Function returns the dissociation constant of hydrogen sulfate
    !            (bisulfate)
