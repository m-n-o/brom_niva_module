...
contains
  ...
  !
    ...
    _LOOP_BEGIN_
        ...
        !Mn2+ + HS- -> MnS(s) + H+
        mns_form = self%K_mns_form*pos(Om_MnS-1._rk)
        !MnS(s) + H+ -> Mn2+ + HS-
        mns_diss = self%K_MnS_diss*MnS*pos(1._rk-Om_MnS)
        !MnS + 2O2 -> Mn(2+) + SO4(2-)
        mns_ox = self%K_mns_ox*MnS*O2
        !MnCO3 precipitation/dissolution
        Om_MnCO3 = Mn2*CO3/(self%K_MnCO3)
        !Mn2+ + CO3-- <-> MnCO3 (vanCappelen,96)
        mnco3_form = self%K_mnco3_form*pos(Om_MnCO3-1._rk)
        mnco3_diss = self%K_mnco3_diss*MnCO3*pos(1._rk-Om_MnCO3)
        ...
        DcDOML_Mn3 = pos(self%K_DOML_mn3*DOML*Mn3 &
                     *thr_h(self%s_mnrd_mn3,Mn3,1._rk)&
                     *thr_l(self%O2s_dn,o2,1._rk))
        
        DcPOML_Mn3 = pos(self%K_POML_mn3*POML*Mn3 &
                     *thr_h(self%s_mnrd_mn3,Mn3,1._rk)&
                     *thr_l(self%O2s_dn,o2,1._rk))
        
        DcDOMR_Mn3 = pos(self%K_DOMR_mn3*DOMR*Mn3 &
                     *thr_h(self%s_mnrd_mn3,Mn3,1._rk)&
                     *thr_h(self%s_OM_refr,DOMR,0.1_rk) &
                     *thr_l(self%O2s_dn,o2,1._rk))
                        
        DcPOMR_Mn3 = pos(self%K_POMR_mn3*POMR*Mn3 &
                     *thr_h(self%s_mnrd_mn3,Mn3,1._rk) &
                    *thr_h(self%s_OM_refr,POMR,0.1_rk) &
                    *thr_l(self%O2s_dn,o2,1._rk))
        
        DcDOML_Mn4 = pos(self%K_DOML_mn4*DOML*Mn4 &
                     *thr_h(self%s_mnrd_mn4,Mn4,1._rk) &
                     *thr_l(self%O2s_dn,o2,1._rk))
        
        DcPOML_Mn4 = pos(self%K_POML_mn4*POML*Mn4 &
                     *thr_h(self%s_mnrd_mn4,Mn4,1._rk) &
                     *thr_l(self%O2s_dn,o2,1._rk))

        DcPOMR_Mn4 = pos(self%K_POMR_mn4*POMR*Mn4 &
                    *thr_h(self%s_mnrd_mn4,Mn4,1._rk) &
                    *thr_h(self%s_OM_refr,POMR,0.1_rk) &
                    *thr_l(self%O2s_dn,o2,1._rk))
        
        DcDOMR_Mn4 = pos(self%K_DOMR_mn4*DOMR*Mn4 &
                    *thr_h(self%s_mnrd_mn4,Mn4,1._rk) &
                    *thr_h(self%s_OM_refr,DOMR,0.1_rk) &
                    *thr_l(self%O2s_dn,o2,1._rk))
        ...
        d_PO4_Mn3 = 0.0_rk !- PO4_Mn3 &  ! old (last time step) PO4_Mn3
